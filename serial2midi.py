...
spCombo = sg.Combo(spStrings,    size=csize,
                   default_value=sg.UserSettings().get(spSettings, ''))
bdCombo = sg.Combo(bdValues,     size=csize,
                   default_value=sg.UserSettings().get(bdSettings, ''))
s2mCombo = sg.Combo(midiinPorts,  size=csize,
                    default_value=sg.UserSettings().get(s2mSettings, ''))
m2sCombo = sg.Combo(midioutPorts, size=csize,
                    default_value=sg.UserSettings().get(m2sSettings, ''))
scKey = '-SCAN-'
stKey = '-START-'
exKey = '-EXIT-'
scButton = sg.Button(scbString, size=bsize, key=scKey,
                     tooltip='Scan for Serial and MIDI ports')
stButton = sg.Button(stbString, size=bsize, key=stKey,
                     tooltip='Start/stop the Serial-MIDI bridging')
    ...
    if event == sg.WIN_CLOSED or event == exKey:
        ...
        if enabled:
            ...
            try:
                ...
                print('Starting: "'+spPortnames[spi]+'" "'+str(
                    bdValues[bdi])+'" "'+midiinPorts[s2mi]+'" "'+midioutPorts[m2si]+'"')
                ok = startSerialMidiServer(
                    spPortnames[spi], bdValues[bdi], s2mi, m2si)
                if ok:
